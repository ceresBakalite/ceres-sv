    ...
    window.customElements.define('ceres-sv', class extends HTMLElement
    {
        async connectedCallback()
        {
            ...
            function configureAttributes()
            {
                ...
                cfg.commaCodes = new RegExp(',|&comma;|&#x2c;|&#44;|U+0002C|%2C', 'g');
                cfg.commaSymbol = '_&c';
                cfg.shadowStyle = '';
                cfg.attrib = {};
                cfg.slide = 1;
                ...
                (function() {
                    ...
                    this.parseText = function(text)
                    {
                        ...
                        let doc = new DOMParser().parseFromString(text.replace(cfg.commaCodes, cfg.commaSymbol).replace(/^\s*?<template(.*?)>|<\/template>\s*?$/, ''), 'text/html');
